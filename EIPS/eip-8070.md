...
### Ticket Sale
Per block `MAX_BLOBS_PER_BLOCK` tickets are sold. Users send regular transactions to the `BLOB_TICKETS_SHOP` contract that specify 1) sender (`sender`) of the future sender of the blob(s), 2) the number of tickets they wish to buy (`num_tickets`). Nodes credit the `sender` with `num_tickets` in their local registry. Transactions interacting with `BLOB_TICKETS_SHOP` are treated sequentially in order of priority fee per ticket. Each transaction receives `num_tickets` as long as the total number of tickets sold is less than `MAX_BLOBS_PER_BLOCK`. If there is a transaction that requests a larger `num_tickets` than tickets are left, it receives the number of tickets that are left. Note that it is unlikely more than `MAX_BLOBS_PER_BLOCK` tickets will be requested as `TARGET_BLOBS_PER_BLOCK` is strictly lower than `MAX_BLOBS_PER_BLOCK`. In case an adversary wants to buy all tickets, it has to compete in a first-price auction against other blob submitters.
