...
### System Contract Predeploy
Upon activation of this EIP, the account at `BLOB_TICKET_SHOP` becomes a contract that functions as a sink. The contract has no functionality except for being a sink.

### Ticket Sale
Per block `MAX_BLOBS_PER_BLOCK` tickets are sold. Users send regular transactions to the `BLOB_TICKETS_SHOP` contract that specify 1) sender's address (`sender`) of the future sender of the blob(s), 2) the number of tickets they wish to buy (`num_tickets`). Nodes credit the `sender` with `num_tickets` in their local registry. Transactions interacting with `BLOB_TICKETS_SHOP` are treated sequentially in order of priority fee per ticket. Each transaction receives `num_tickets` as long as the total number of tickets sold is less than `MAX_BLOBS_PER_BLOCK`. If there is a transaction that requests a larger `num_tickets` than tickets are left, it receives the number of tickets that are left. Note that it is unlikely more than `MAX_BLOBS_PER_BLOCK` tickets will be requested as `TARGET_BLOBS_PER_BLOCK` is strictly lower than `MAX_BLOBS_PER_BLOCK`. In case an adversary wants to buy all tickets, it has to compete in a first-price auction against other blob submitters.

### Local Node Registry
Each node keeps a local record of how many tickets each `sender` holds and when they expire. The local registry is updated each slot when new tickets are bought. Nodes allocate tickets to transactions in order of priority fee per ticket. Specifically, each node computes off-chain the priority fee divided by the `num_tickets` requested by a transaction and allocates tickets in descending order. In order to accept a `blob_tx`, its `sender` must have at least `len(blob_tx.blob_versioned_hashes)` tickets active in the node's local registry, which are deducted from the `sender`'s active ticket balance. Tickets expire at `buying_slot + SLOTS_TO_TICKET_EXPIRY` and are deducted from the `sender`'s ticket balance. `buying_slot` is `block.slot` for the block where the ticket purchase transaction appeared.

Concretely a node keeps a dictionary with `sender` keys and corresponding values being double-ended queues of "batches". A `Batch=(expiry_slot,remaining_tickets)` is a bundle of tickets sharing the same expiry and is represented by the pair of integers `expiry_slot` and `remaining_tickets` corresponding to the number of tickets expiring at a given slot. When  `sender` acquires `num_tickets` in `buying_slot`, a `Batch=(buying_slot + SLOTS_TO_TICKET_EXPIRY, num_tickets)`gets added to the deque and gets processed from left to right when tickets are being used by `sender` (so that older tickets get used first). In each slot the queue moves one step and after `SLOTS_TO_TICKET_EXPIRY`the `Batch`gets popped off.
...
- **Slots to expiry**: `SLOTS_TO_TICKET_EXPIRY = 2`
