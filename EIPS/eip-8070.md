...
### Ticket Sale
Per block `MAX_BLOBS_PER_BLOCK` tickets are sold. Users send regular transactions to the `BLOB_TICKETS_SHOP` contract that specify 1) public key (`pk`) of the future sender of the blob(s), 2) the number of tickets they wish to buy (`num_tickets`). Nodes credit the `pk` with [`num_tickets`, `expiry_slot`] in their local registry. `expiry_slot` is `d` slots later than the current slot. Transactions interacting with `BLOB_TICKETS_SHOP` are treated sequentially in order of priority fee per ticket. Each transaction receives `num_tickets` as long as the total number of tickets sold is less than `MAX_BLOBS_PER_BLOCK`. If there is a transaction that requests a larger `num_tickets` than tickets are left, it receives the number of tickets that are left. Note that it is unlikely more than `MAX_BLOBS_PER_BLOCK` tickets will be requested as `TARGET_BLOBS_PER_BLOCK` is strictly lower than `MAX_BLOBS_PER_BLOCK`. In case an adversary wants to buy all tickets, it has to compete in a first-price auction against other blob submitters.

### Local Node Registry
Each node keeps a local record of how many tickets each public key holds and when they expire. The local registry is updated each slot when new tickets are bought. Nodes allocate tickets to transactions in order of priority fee per ticket. Specifically, each node computes off-chain the priority fee divided by the `num_tickets` requested by a transaction and allocates tickets in descending order. A ticket is deducted when the node pulls cells from a blob sent by a `pk` with more than zero valid blob tickets. If the current `slot` is larger than the `expiry_slot`, then `num_tickets` that are associated with `expiry_slot` are deducted.
